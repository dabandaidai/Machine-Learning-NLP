...
def log_likelihood(images, theta, pi):
    ...
    # Now for denominator
    exponential = np.array(e)
    grand = exponential ** numerator
    outer_sum = np.sum(grand, axis=1)
...
def predict(log_like):
    ...
    # Finds the maximum of each row, which is the class that the image belongs since it is binary
...
def accuracy(log_like, labels):
    ...
    # The actual class for each image
    labels_index = np.argmax(labels, axis=1)
    # The predictions for each image
    predictions = predict(log_like)
    # Find examples where actual matches prediction
    equals = np.equal(labels_index, predictions)
    correct = np.sum(equals)
    result = correct / log_like.shape[0]
    return result
